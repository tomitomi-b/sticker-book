...
async function onRequest2(context) {
  ...
  try {
    ...
    if (!userId) return cors2(json2({ error: "userId \u304C\u5FC5\u8981\u3067\u3059" }, 400));
    ...
    return cors2(json2({ collection: rows.results || [] }));
  } catch (e) {
...
async function onRequest3(context) {
  ...
  try {
    ...
    return cors3(json3({ error: "Unknown action" }, 400));
  } catch (e) {
    return cors3(json3({ error: String(e.message || e) }, 500));
  }
}
...
async function updatePresence(request, env) {
  ...
  if (!userId || lat == null || lng == null) {
    return cors3(json3({ error: "userId, lat, lng \u304C\u5FC5\u8981\u3067\u3059" }, 400));
  }
  await env.DB.prepare(
    `INSERT INTO users (id) VALUES (?) ON CONFLICT(id) DO NOTHING`
  ).bind(userId).run();
  ...
  return cors3(json3({ ok: true }));
}
__name(updatePresence, "updatePresence");
async function leaveExchange(request, env) {
  const body = await request.json();
  const { userId } = body;
  if (!userId) return cors3(json3({ error: "userId \u304C\u5FC5\u8981\u3067\u3059" }, 400));
...
async function getNearby(request, env) {
  ...
  const nearby = (rows.results || []).map((r) => {
    const distance = haversine2(lat, lng, r.lat, r.lng);
    return {
      userId: r.user_id,
...
async function createOffer(request, env) {
  ...
  if (!fromUserId || !toUserId || !offerSpotId || !requestSpotId) {
    return cors3(json3({ error: "\u5FC5\u9808\u9805\u76EE\u304C\u4E0D\u8DB3\u3057\u3066\u3044\u307E\u3059" }, 400));
  }
  if (fromLat != null && fromLng != null && toLat != null && toLng != null) {
    const dist = haversine2(fromLat, fromLng, toLat, toLng);
    if (dist > 80) {
      return cors3(json3({ error: "\u76F8\u624B\u3068\u96E2\u308C\u3059\u304E\u3066\u3044\u307E\u3059", distance_m: Math.round(dist) }, 403));
    }
  }
  ...
  if (!fromOwns) return cors3(json3({ error: "\u6E21\u3059\u30B7\u30FC\u30EB\u3092\u6301\u3063\u3066\u3044\u307E\u305B\u3093" }, 400));
  if (!toOwns) return cors3(json3({ error: "\u76F8\u624B\u304C\u305D\u306E\u30B7\u30FC\u30EB\u3092\u6301\u3063\u3066\u3044\u307E\u305B\u3093" }, 400));
  ...
  if (fromHasRequest) return cors3(json3({ error: "\u3059\u3067\u306B\u53D7\u3051\u53D6\u308A\u305F\u3044\u30B7\u30FC\u30EB\u3092\u6301\u3063\u3066\u3044\u307E\u3059" }, 400));
  if (toHasOffer) return cors3(json3({ error: "\u76F8\u624B\u306F\u3059\u3067\u306B\u305D\u306E\u30B7\u30FC\u30EB\u3092\u6301\u3063\u3066\u3044\u307E\u3059" }, 400));
  ...
  return cors3(json3({ offerId: id, status: "pending" }));
}
...
async function respondOffer(request, env) {
  ...
  if (!fromHasOffer || !toHasRequest) {
    ...
    return cors3(json3({ error: "\u30B7\u30FC\u30EB\u306E\u6240\u6301\u72B6\u614B\u304C\u5909\u308F\u3063\u3066\u3044\u307E\u3059" }, 409));
  }
  await env.DB.batch([
  ...
  return cors3(json3({ status: "accepted" }));
}
...
async function listOffers(request, env) {
  ...
  return cors3(json3({ offers: rows.results || [] }));
}
  ...
  try {
    if (request.method === "POST") {
      ...
      if (!userId || typeof userId !== "string" || userId.length < 8) {
        return cors4(json4({ error: "userId \u304C\u4E0D\u6B63\u3067\u3059" }, 400));
      }
      await env.DB.prepare(
      ...
      return cors4(json4({ user }));
    }
    if (request.method === "GET") {
      const url = new URL(request.url);
      const userId = url.searchParams.get("userId");
      if (!userId) return cors4(json4({ error: "userId \u304C\u5FC5\u8981\u3067\u3059" }, 400));
      const user = await env.DB.prepare("SELECT id, display_name, created_at FROM users WHERE id = ?").bind(userId).first();
      return cors4(json4({ user: user || null }));
    }
...
async function onRequest5(context) {
  ...
  try {
    ...
    if (!Number.isNaN(lat) && !Number.isNaN(lng)) {
      list = list.map((s) => {
        const distance = haversine3(lat, lng, s.lat, s.lng);
        return {
          ...s,
          distance_m: Math.round(distance),
...
// ../.wrangler/tmp/pages-h6PKJb/functionsRoutes-0.7156535608306626.mjs
var routes = [
  {
    routePath: "/api/claim",
    mountPath: "/api",
