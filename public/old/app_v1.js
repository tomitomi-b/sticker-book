...
async function openTradeModal(toUserId) {
  // 自分の所持と相手の所持を取得して選択UI
  const myCol = await api('/collection?userId=' + encodeURIComponent(state.userId));
  // 相手の所持は公開APIがないので、簡易版：自分の所持から「渡す」、相手が持っていそうなものは後で拡張
  // MVP: 自分の所持から「渡すシール」、スポット一覧から「欲しいシール」を選ぶ（相手が持っているかはサーバーで検証）
  ...
  backdrop.querySelector('#modal-ok').onclick = async () => {
    ...
    try {
      // 相手の位置は unknown なので、自分の位置だけ送る（近接はプレゼンス側で担保）
